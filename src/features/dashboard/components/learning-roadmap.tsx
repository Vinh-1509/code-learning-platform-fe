...
import { CheckCircle2, Play, Lock, ChevronDown, ChevronUp, Map } from "lucide-react"
  ...
  return (
    <div
      className={cn(
        ...
        // 👇 ĐÃ FIX: Thay border-primary/40 bg-primary/5 thành viền xám nhẹ, nền trắng tinh cho tiệp với Figma
...
function ModuleItem({ module, isExpanded, onToggle, onLessonComplete }: ModuleItemProps) {
  const completedLessons = module.lessons.filter(l => l.status === "done").length
      ...
      <div className="flex flex-col items-center">
        ...
        {module.id < 4 && (
          <div className={cn(
            ...
            completedLessons === module.lessons.length ? "bg-primary" : "bg-border"
          )} />
        )}
      </div>
...
export function LearningRoadmap() {
    ...
    setModules(prev => {
      ...
      for (let i = 0; i < newModules.length; i++) {
        for (let j = 0; j < newModules[i].lessons.length; j++) {
          if (newModules[i].lessons[j].id === lessonId && newModules[i].lessons[j].status === "current") {
  ...
  return (
    <Card className="bg-card border-2 border-primary/40 shadow-[0_4px_20px_rgba(59,130,246,0.08)] py-0 gap-0">
      <CardHeader className="p-6 pb-4 gap-0">
